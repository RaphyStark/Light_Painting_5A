...
void master() {
    ...
    while (failure < 6) {
        ...
        payload=valeur_V();
        payload2=valeur_Omega();
       
        bool report = radio.write(&payload, sizeof(float));         // transmit & save the report
        bool report2 = radio.write(&payload2, sizeof(float)); 
        if (report) {
        ...
        if (report2) {
            // payload was delivered
            cout << "Transmission 2 successful! Time to transmit = ";
            cout << timerEllapsed;                                  // print the timer result
            cout << " us. Sent: " << payload2 << endl;               // print payload sent
                                              

        } else {
            // payload was not delivered
            cout << "Transmission 2 failed or timed out" << endl;
            failure++;
        }
