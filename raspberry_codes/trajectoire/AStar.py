...
class Map:
        ...
        for y in range(0,self.dimY):
            for x in range(0,self.dimX):
                ...
                if (self.occupancy[x,y]==0): # connetivity only for node with no obstacles
                    ...
                    # passage à 1 pour les noeuds voisins en degré d'adjacence de 4
                    ...
                    # passage à 1 pour les noeuds voisins en degré d'adjacence de 8
    ...
    def builtPath(self, closedList):
        ...
        first_node = cList[0]
        last_node = cList[len(cList)-1]
        parent = []
        #current_node = last_node
        parent.append(last_node)
        while True:
            parent.append(self.graph.listOfNodes[last_node].parentNo)
            last_node = self.graph.listOfNodes[last_node].parentNo
            if self.graph.listOfNodes[last_node].parentNo == first_node :
                break
...
def manhattanDist(node1, node2):
    distance = 0.0
    distance = abs(node2.x - node1.x) + abs(node2.y - node1.y)
    return distance
...
if __name__=='__main__':
    ...
    print("path : " + str(path))
    print("longueur du path : " + str(lenpath))
